...
系统必须为每个账号维护一个可空的「起算日期」水位线 `processFrom`（`DateTime?`，**UTC 语义**）。`processFrom` 为 NULL 时表示**不设日期下界**（保持既有全量行为）。该水位线**禁止**复用 `lastSyncCursor`（归 IMAP UID 游标语义）或 `createdAt`（审计语义）——必须是独立列。存储为 Postgres `TIMESTAMP`（Prisma 按 UTC 读写），与 `receivedAt` 同类型，可直接比较、无隐式转换。
...
种值机制**归 repo 的行创建分支**、`update` 分支一律不动该列（使「seed-on-create / preserve-on-update / 只有 set-process-from 改既有」结构成立、CLI 无需分辨首次 vs re-auth）：create 分支（`createAccount` 与 `upsertAccount.create`）写 `input.processFrom ?? new Date()`——默认是**精确瞬时 `new Date()`**（**非** UTC 零点；UTC 零点仅用于显式 date-string，避免默认 seed 凭空提前 ≤24h）；`upsertAccount` 的 `update` 分支**一律不含** `processFrom`（Prisma = 列不动；InMemory 须 **get-before-set**：existing 存在则保留 `existing.processFrom`、**忽略** input，不存在则用 `input.processFrom ?? new Date()`，读必须先于 set）。因此对**既有**账号 `add --process-from` 也**不**改水位线（走 update 被忽略）；**只有 `set-process-from`** 能改既有行。`AccountWriteInput` 新增 `processFrom?` 作播种载体。
...
#### 场景:默认 seed 用精确瞬时、不被抹到 UTC 零点
- **当** 接入未给 `--process-from`
- **那么** `processFrom` 必须为精确 `new Date()` 瞬时；UTC 零点解析**仅**作用于显式 date-string

#### 场景:显式覆盖起算日期
- **当** 运维 `account add … --process-from <ISO date>`
- **那么** 新行的 `processFrom` 必须为该日期（解析为 UTC 零点，经 3.1 共享 helper、含未来日期拒绝）
...
`<date>` 的解析与校验必须经**一个共享 helper**，`account add --process-from` 与 `set-process-from` **都**用它（两命令同一脚枪、同一守卫）：解析器**严格**只接受 `^\d{4}-\d{2}-\d{2}$` 形式、经 `Date.UTC(y, m-1, d)` 落 **UTC 零点**；**禁止**用 `new Date(str)` 的宽松解析（会放过带时间分量 `…T14:00` 或 `2026-13-99` 这类被强转的串、破坏「UTC 零点」保证）；不匹配 / `Invalid Date` → 用法错误（退出码 2）。**严格未来**（`parsedUtcMidnight > now`）→ 用法错误（否则静默排除该日前**所有**邮件含合法新邮件）；判定按 `parsedUtcMidnight > now`（非 date-to-date），使**今天**（UTC 零点 ≤ now）在任意时区不被误拒。

`set-process-from` 是**无条件覆盖**（可双向移动水位线、无单调守卫）：把既有 `processFrom` 改为给定 UTC 零点日期——若该日期**早于**既有值（如对当日 14:30 接入的账号盖 `<当日>` 的 UTC 零点 00:00），水位线**前移**、会**重新纳入**此前被排除的当日上午邮件。故运维压积压须盖到**接入处理日之后**（见 runbook），而非当日。

#### 场景:运维盖戳存量账号压住历史积压
- **当** 运维对一个有历史积压的存量账号执行 `account set-process-from <id> <今天>`
- **那么** 系统必须把其 `processFrom` 设为该 UTC 日期；此后该账号 `receivedAt < processFrom` 的旧邮件不再被摄入、也不再进摘要
...
#### 场景:非法日期被拒绝
- **当** 运维执行 `account set-process-from <id> <不可解析的日期>`
- **那么** 系统必须以用法错误（退出码 2）拒绝，**禁止**静默写入或回落

#### 场景:未来日期在两命令都被拒绝
- **当** 运维执行 `set-process-from <id> <晚于 now 的日期>` **或** `account add … --process-from <未来日期>`
- **那么** 系统必须经共享 helper 以用法错误（退出码 2）拒绝（防静默排除所有邮件直到该未来日）

#### 场景:今天不被误拒
- **当** 运维传 `--process-from <今天的日期>`（其 UTC 零点 ≤ now，含东于 UTC 的时区）
- **那么** 必须按 `parsedUtcMidnight > now` 判为**非未来**、放行（不被 date-to-date `>=` 误拒）
