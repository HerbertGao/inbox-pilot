...
两腿读 overlay 时**不得**在 loader 的逐行归一（trim + 小写）之上再叠一层（如剥 `<>`）。曾在两腿各加一层归一以消除彼此的不对称，结果两腿一致了却与 loader 分道扬镳：存量行 `<alice@example.com>` 在 loader 眼里是一条独立死行，在两腿眼里是 `alice@example.com`，于是用户想加这个地址时得到「已在名单里」的回执、文件没动、邮件照旧不降噪。**同一份文件被两个消费者读成不同的集合，是比两腿不对称更坏的一类 bug。**
