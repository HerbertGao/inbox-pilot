...
- **P4 需内容层欺骗证据**：**仅当**邮件**内容**含「诱导收件人按欺骗前提行动」的欺骗证据（如冒充身份诱导点链接 / 转账 / 交出验证码；含冒充上级/财务的紧急转账 BEC，发件人正常/无链接也算）才归 P4；**表层信号**——可疑 TLD（如 `.xyz`）、文本截断、未渲染的模板变量、`return-path` 单独出现（尤其命中自有转发域）——**禁止**单独构成 P4。
- **P0 不再一刀切**：**去除**「交易 / 告警」无差别归 P0 的指示；交易**收据**类、系统**例行**通知归 P2/P3；**账号/登录类安全提醒**（登录活动、新设备登录、应用专用密码、sign-in 复核等「若本人操作可忽略」者）归 P1/P2，仅确认异常才 P0；P0 收窄为「真正需要收件人立即亲自动手处理」者。
...
- **当** 构造分类 prompt
- **那么** prompt 必须指示「P4 需内容层欺骗证据」（含 BEC 冒充转账），并明确可疑 TLD / 截断 / 未渲染模板 / 单独 return-path 等表层信号**不**单独构成 P4

#### 场景:P0 不再一刀切交易/告警
- **当** 构造分类 prompt
- **那么** prompt 必须不再把「交易 / 告警」无差别归 P0；交易收据 / 例行通知 / 账号登录类安全提醒归 P1/P2/P3、仅确认异常才 P0
...
守门**两侧语义非对称**：**真钓鱼集为安全线、逐样本守门**（MUST，任一掉出即失败——本期实现为 k=5 多数表决、某样例 <3/5 落 P4 即失败，区分单次抖动与系统性误伤）；**误报集为噪音线、允许通过率阈值**（SHOULD，≥阈值降出 P0/P4 即通过，吸收 LLM 抖动）。守门的**运行机制**（cadence、阈值、语料存放、CI 凭据、语料代表性）属实现细节、不入本需求；本需求只钉**行为属性**。该守门**独立于**规则引擎硬底线：无论 prompt 如何误评，规则引擎对 P4/敏感邮件「不自动标已读」的硬底线**仍**生效。

**残留诚实边界**：真钓鱼安全线的强度受**语料覆盖**约束——若缺「表层弱、内容欺骗强」的真脱敏样例（如部署侧上游已过滤钓鱼、无真样例可采），安全线在该类上欠武装、为 best-effort，由引擎硬底线 + `confidence<0.65` 兜底。
