...
P0 必需变量集必须**仅**为：`DATABASE_URL`（无默认值，缺失即 fail-fast；须为合法的 postgres 连接串，scheme 为 `postgresql://` 或 `postgres://`，否则 fail-fast）。所有后续阶段变量（`OPENROUTER_*`、`GMAIL_*`、`DIGEST_*`——即 schema 里仍有字段的那些；`TELEGRAM_*` / `BARK_*` / `POLL_*` 已无字段，落在「不纳入校验」一侧）在 P0 必须为可选或不纳入校验，**禁止**因其缺失而拒绝启动。
